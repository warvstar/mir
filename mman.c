...
void *mmap (void *addr, size_t len, int prot, int flags, int fildes, off_t off) {
  HANDLE fm, h;
  return (void *) VirtualAlloc (0, len, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
  void *map = MAP_FAILED;
...
#pragma warning(disable : 4293)
  ...
  const DWORD dwMaxSizeHigh
    = (sizeof (off_t) <= sizeof (DWORD)) ? (DWORD) 0 : (DWORD) ((maxSize >> 32) & 0xFFFFFFFFL);
...
int mprotect (void *addr, size_t len, int prot) {
  DWORD newProtect = PAGE_EXECUTE_READWRITE;  //__map_mmap_prot_page (prot);
  DWORD oldProtect = 0;
